...
type PendingAction = Mode | Provider;
...
export function AccountPanel({ userEmail }: { userEmail: string }) {
  return <Panel account userEmail={userEmail} />;
}
    ...
    >
      <Card className="w-full max-w-xl gap-0 border-0 bg-card py-0 shadow-2xl ring-1 ring-foreground/10">
        <CardContent
          ...
          aria-busy="true"
        >
          <Skeleton className="h-11 w-20" aria-hidden="true" />
          <Skeleton className="mt-4 h-11 w-3/4" aria-hidden="true" />
          <Separator className="my-7" />
          <Skeleton className="h-5 w-full" aria-hidden="true" />
          <Skeleton className="mt-3 h-5 w-2/3" aria-hidden="true" />
          <div className="mt-7 grid gap-3 sm:grid-cols-2">
            <Skeleton className="h-11 w-full rounded-full" aria-hidden="true" />
            <Skeleton className="h-11 w-full rounded-full" aria-hidden="true" />
          </div>
        </CardContent>
      </Card>
...
function Panel({ account = false, userEmail = "" }: { account?: boolean; userEmail?: string }) {
  ...
  async function linkProvider(provider: Provider) {
    ...
    try {
      const result = await authClient.linkSocial({
        provider,
        callbackURL: `${window.location.origin}/account?linked=${provider}`,
      });
      if (result.error)
        setMessage({
  ...
  if (account) {
    return (
      <AuthFrame title="Account settings" app>
...
function AuthFrame({
  ...
  return (
    <main
      id="main-content"
      className={`flex items-center justify-center bg-background px-5 py-10 sm:px-8 ${
        app ? "min-h-[calc(100vh-5.25rem)]" : "min-h-screen"
      }`}
    >
      <Card className="w-full max-w-xl gap-0 border-0 bg-card py-0 shadow-2xl ring-1 ring-foreground/10">
        <CardContent className="p-7 sm:p-10">
          ...
          <h1
            id="auth-title"
            className={`${app ? "" : "mt-4"} text-4xl font-semibold leading-[0.94] tracking-[-0.065em] sm:text-5xl`}
          >
            {title}
          </h1>
          <Separator className="my-7" />
